## Hi there 👋

# 👩‍🔬 Hi, I’m Mercy Francis

🎯 **Precision-Driven Data Analyst** | 🔬 Med Biochemist-Turned-Data Explorer  
🌍 Nigeria-based | 🧬 Specializing in Science-Driven Data Analytics

---

## 🧠 About Me

I extract clarity from complexity—cleaning, analyzing, and visualizing data with surgical precision.  
With roots in Medical biochemistry and hands-on lab work, I now translate biomedical and business data into insights using:

- 🔍 **Excel & Power Query** for agile data wrangling  
- 🧮 **PostgreSQL** for structured querying  
- 📊 **Power BI** for live dashboards  
- 🐍 Learning **Python** for automation
- 🔵**R** Learning **R** for statistical computing and visualization

---

## 🧬 Bioinformatics & Clinical Data Projects

### 🧠 [Glioblastoma Biomarker Discovery](https://github.com/MercyOFrancis/hackbio-cancer-internship)
Analyzed gene expression data for glioma classification  
- Performed differential expression analysis using R  
- Built heatmaps to highlight up/downregulated genes  
- Enriched biological pathways using ShinyGO  

### 💡 [Colorectal Cancer Classifier](https://github.com/MercyOFrancis/hackbio-cancer-internship)
- Cleaned raw gene expression profiles  
- Supported ML team in building a kNN-based classifier  
- Identified significant biomarkers from expression data  

---

## 📈 Featured Work in Business Analytics

### 📊 [Excel | Job Market Insights](https://github.com/MercyOFrancis/MercyOFrancis/tree/main/EXCEL%20projects/Data%20Cleaning%20-%20Job%20Posting)
- Processed 15K+ job posts using Power Query  
- Created interactive dashboards & KPIs  
- Boosted data integrity by **95%+**

### 🗃️ [SQL | Retail Analytics Engine](https://github.com/MercyOFrancis/MercyOFrancis/tree/main/SQL%20projects)
- Designed & queried relational databases  
- Applied CTEs, joins, and window functions for segmentation  
- Cut query time by **90%** through optimization

---

## 🛠️ Tools I Use

![Excel](https://img.shields.io/badge/-Excel-217346?style=flat-square&logo=microsoft-excel&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white)
![Power BI](https://img.shields.io/badge/-Power_BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=r&logoColor=white)

---

## 🎓 Certifications & Training

- 🧬 **HackBio Internship** – Gene Expression & Biomarker Analysis (2024)  
- 📊 **Data Analysis with Excel** – DataCamp (2024)  
- 🧠 **SQL Associate Track** – DataCamp (2025)  
- 🛠️ **Python & Power BI** (In Progress)
-  🧬 **R** (In Progress)

---

## 📫 Let’s Connect

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/mercy-francis)  
[![Email](https://img.shields.io/badge/-Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mercyfrancis18@gmail.com)  
[![Portfolio](https://img.shields.io/badge/-GitHub%20Portfolio-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/MercyOFrancis)

---

## 💡 Fun Facts

- 🌍 crunching datasets from every corner of the globe  
- 🎬 Recharges with binge-worthy series and lively board- or card-game nights  
- 🎾 Weekend tennis learner & casual volleyball setter—minus the sore knuckles  
- ☕ Powered by yoghurt, café-hopping, and the hunt for fancy desserts  
- 🛍️ Loves spontaneous travel and retail therapy—passport and shopping cart always ready  
- 🎯 Mission: turn complex data into stories everyone can act on

---

## 🧪 From Bench to Bytes

Previously worked as a **Lab Assistant at Clinix Healthcare**, managing pathology sample data and lab equipment like ELISA, HPLC, and PCR machines.  
Now leveraging that structured rigor to optimize pipelines and uncover **hidden patterns in life sciences & business data**.

---

## 📊 Stats & Streaks

![Mercy's GitHub Stats](https://github-readme-stats.vercel.app/api?username=MercyOFrancis&show_icons=true&theme=radical&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=MercyOFrancis&layout=compact&theme=radical&hide_border=true)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=MercyOFrancis&theme=radical&hide_border=true)

---

> _“Refining data is like refining DNA—accuracy reveals function.”_  
Let’s bring order to your chaos.

---
⭐ **If you find my work interesting, please consider giving my repositories a star!**

![Profile Views](https://komarev.com/ghpvc/?username=MercyOFrancis&color=blueviolet&style=flat)

---
<div align="center">
  
### 🚀 Ready to turn your data into insights? Let's collaborate!

</div>
